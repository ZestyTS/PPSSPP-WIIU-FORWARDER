...
bool exist_file (const std::string &s)
{
  struct stat buffer;
  return (stat (s.c_str(), &buffer) == 0);
}

void b2hex(uint64_t input, char* output) {
...
WiiUHost::WiiUHost() {
	...
	/*uint64_t tID;
   	tID = OSGetTitleID();
   	char titleIDHex[16];
   	b2hex(tID, titleIDHex);
   	char usbPath[] = "storage_usb:/usr/title/00050002/XXXXXXXX/content/";
   	memcpy(&usbPath[23], titleIDHex, 8);
   	memcpy(&usbPath[32], titleIDHex + 8, 8);
  
   	if (!exist_file(usbPath)) {
